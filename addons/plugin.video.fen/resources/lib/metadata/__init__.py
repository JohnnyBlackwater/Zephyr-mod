...
movie_result = tmdb.tmdbMoviesExternalID
tvshow_data = tmdb.tmdbTVShows
tvshow_result = tmdb.tmdbTVShowsExternalID
...
def movie_meta(id_type, media_id, user_info):
	def no_data():
		...
		metacache.set('movie', meta, timedelta(hours=48))
		return meta
	def tmdb_meta(language):
		return movie_data(media_id, language, tmdb_api) if id_type == 'tmdb_id' else movie_data(movie_result(id_type, media_id, tmdb_api)['id'], language, tmdb_api)
	...
	def set_cache_meta():
		metacache.set('movie', meta, timedelta(hours=4368))
	def delete_cache_meta():
		metacache.delete('movie', 'tmdb_id', meta['tmdb_id'])
	def check_tmdb_data(data):
		if language != 'en' and data['overview'] in ('', None, 'None'):
...
def tvshow_meta(id_type, media_id, user_info):
	def no_data():
		...
		metacache.set('tvshow', meta, timedelta(hours=48))
		return meta
	def tmdb_meta():
		return tvshow_data(media_id, language, tmdb_api) if id_type == 'tmdb_id' else tvshow_data(tvshow_result(id_type, media_id, tmdb_api)['id'], language, tmdb_api)
	def fanarttv_meta(fanart_id):
	...
	def set_cache_meta(status):
		if status in finished_show_check: hours = 4368
		else: hours = 96
...
def season_episodes_meta(season, meta, user_info):
	...
	if data: return data
	try:
		if meta['status'].lower() in finished_show_check: hours = 4368
		...
		metacache.set('season', data, timedelta(hours=hours), string)
	except: pass
	return data
...
def build_movie_meta(data, fanarttv_data=None):
	...
	plot = to_utf8(get('overview', ''))
	tagline = to_utf8(get('tagline', ''))
	votes = get('vote_count', '')
	premiered = get('release_date', '')
	if get('poster_path'): poster = 'https://image.tmdb.org/t/p/%s%s' % (poster_resolution, data['poster_path'])
	else: poster = ''
	if get('backdrop_path'): fanart = 'https://image.tmdb.org/t/p/%s%s' % (fanart_resolution, data['backdrop_path'])
	else: fanart = ''
	if fanarttv_data:
		fanart_added = True
	...
	ei_status = get('status', 'N/A')
	ei_homepage = get('homepage', 'N/A')
	if get('belongs_to_collection', False):
	...
	return {'tmdb_id': tmdb_id, 'imdb_id': imdb_id, 'rating': rating, 'plot': plot, 'tagline': tagline, 'votes': votes, 'premiered': premiered, 'imdbnumber': imdb_id,
			...
			'discart': discart, 'genre': genre, 'title': title, 'original_title': original_title, 'year': year, 'duration': duration, 'rootname': rootname,
			'country': country, 'country_codes': country_codes, 'mpaa': mpaa, 'cast': cast, 'writer': writer, 'director': director, 'alternative_titles': alternative_titles,
			'all_trailers': all_trailers, 'trailer': trailer, 'studio': studio, 'fanart_added': fanart_added, 'extra_info': extra_info, 'mediatype': 'movie', 'tvdb_id': 'None'}
...
def build_tvshow_meta(data, fanarttv_data=None):
	...
	return {'tmdb_id': tmdb_id, 'tvdb_id': tvdb_id, 'imdb_id': imdb_id, 'rating': rating, 'plot': plot, 'tagline': tagline, 'votes': votes, 'premiered': premiered,
			'poster': poster, 'fanart': fanart, 'poster2': poster2, 'fanart2': fanart2, 'banner': banner, 'clearart': clearart, 'clearlogo': clearlogo, 'landscape': landscape,
			'discart': discart, 'genre': genre, 'title': title, 'original_title': original_title, 'year': year, 'duration': duration, 'rootname': rootname, 'imdbnumber': imdb_id,
			'country': country, 'country_codes': country_codes, 'mpaa': mpaa, 'cast': cast, 'writer': writer, 'director': director, 'alternative_titles': alternative_titles,
			'all_trailers': all_trailers, 'trailer': trailer, 'studio': studio, 'fanart_added': fanart_added, 'extra_info': extra_info, 'mediatype': 'tvshow',
			'season_data': season_data, 'total_seasons': total_seasons, 'total_aired_eps': total_aired_eps, 'tvshowtitle': title, 'status': status}
...
def retrieve_user_info(window=None):
	if not window:
		from modules.kodi_utils import window
