...
from modules.kodi_utils import translate_path, confirm_dialog, window
	...
	def delete(self, db_type, id_type, media_id, dbcon=None, meta_language=None, tvdb_id=None):
		...
		try:
			...
			if db_type  == 'movie':
				...
				dbcur.execute("DELETE FROM function_cache WHERE string_id LIKE ?", ('tmdbSeasons_%s' % str(media_id)+'%',))
			...
			if meta_language:
				dbcur.execute("DELETE FROM function_cache WHERE string_id = ?", ('%s_%s_%s_%s' % ('fanart', fanart_db_type, meta_language, str(fanart_id)),))
...
def cache_function(function, string, url, expiration=96, json=True):
	...
	metacache.set_function(string, result, expiration=datetime.timedelta(hours=expiration))
