...
trakt_db = translate_path('special://profile/addon_data/plugin.video.fen/traktcache.db')
...
def check_databases():
	...
	dbcon.execute("""CREATE TABLE IF NOT EXISTS watched_status
					(db_type text, media_id text, season integer, episode integer, last_played text, title text, unique(db_type, media_id, season, episode))""")
	dbcon.execute("""CREATE TABLE IF NOT EXISTS progress_status
					(db_type text, media_id text, season integer, episode integer, resume_point text, curr_time text, unique(db_type, media_id, season, episode))""")
	dbcon.close()
	# Main Cache
...
def clean_databases(current_time=None, database_check=True, silent=False):
	...
	functions_list = []
	functions_list.append((external_db, command_base % ('results_data', 'expires')))
	functions_list.append((maincache_db, command_base % ('maincache', 'expires')))
	functions_list.append((metacache_db, command_base % ('metadata', 'expires')))
	functions_list.append((metacache_db, command_base % ('function_cache', 'expires')))
	functions_list.append((metacache_db, command_base % ('season_metadata', 'expires')))
	functions_list.append((debridcache_db, command_base % ('debrid_data', 'expires')))
	threads = list(make_thread_list(_process, functions_list, Thread))
	[i.join() for i in threads]
	######################################################################################
	#Just for this update 2.0.45...
	#After this update, remove Line 123 from meta_cache
	dbcon = database.connect(metacache_db, timeout=60.0)
	dbcur = dbcon.cursor()
	dbcur.execute('''PRAGMA synchronous = OFF''')
	dbcur.execute('''PRAGMA journal_mode = OFF''')
	dbcur.execute("DELETE FROM function_cache WHERE string_id LIKE ?", ('tmdbSeasons_%',))
	dbcon.commit()
	dbcur.execute('VACUUM')
	dbcon.close()
	######################################################################################
