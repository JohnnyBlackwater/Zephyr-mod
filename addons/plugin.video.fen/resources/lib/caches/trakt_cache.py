...
dbfile = translate_path('special://profile/addon_data/plugin.video.fen/traktcache.db')
timeout = 60

SELECT = 'SELECT id FROM trakt_data'
SELECT_LIKE = 'SELECT id FROM trakt_data WHERE id LIKE %s'
...
def reset_activity(latest_activities):
	...
	try:
		...
		dbcur = dbcon.cursor()
		dbcur.execute('SELECT data FROM trakt_data WHERE id=?', ('trakt_get_activity',))
		...
		dbcon.commit()
		_cache.set('trakt_get_activity', latest_activities)
	except: pass
	return cached_data
...
def clear_trakt_list_contents_data(clear_all=False, user=None, list_slug=None):
	if clear_all:
		try:
			dbcon = _cache.connect_database()
			dbcur = dbcon.cursor()
			dbcur.execute(DELETE_LIKE % "'trakt_list_contents_%'")
			dbcon.commit()
		except: pass
	else:
...
def clear_all_trakt_cache_data(silent=False, confirm=True):
	def _process():
		dbcon = _cache.connect_database()
		dbcur = dbcon.cursor()
		for table in ('trakt_data', 'progress_status', 'watched_status'):
