...
def torrent_status_browser(files):
	...
	main_cache.set('furk_active_downloads', [i['info_hash'] for i in files], expiration=timedelta(hours=1))
...
def furk_tfile_audio(params):
	__handle__ = int(argv[1])
	kodi_utils.window.clearProperty('furk_t_files_json')
	...
	kodi_utils.window.setProperty('furk_t_files_json', str(t_files_json))
	kodi_utils.add_items(__handle__, list(_builder()))
	kodi_utils.set_content(__handle__, 'files')
	kodi_utils.end_directory(__handle__)
	kodi_utils.set_view_mode('view.premium')
...
def browse_audio_album(t_files, item_path=None):
	...
	if not t_files:
		seperate = True
		t_fs = kodi_utils.window.getProperty('furk_t_files_json')
...
def remove_from_downloads(item_id):
	...
	if response['status'] == 'ok':
		from datetime import timedelta
		main_cache.set('furk_active_downloads', None, expiration=timedelta(hours=1))
		kodi_utils.notification(32576, 3500)
	else:
		kodi_utils.notification(32574, 3500)
	return (None, None)
...
def get_active_downloads():
	...
	if cache != None: result = cache
		...
		main_cache.set('furk_active_downloads', result, expiration=timedelta(hours=1))
	return result
...
def account_info(params):
	try:
		...
		line = '%s[CR]%s'
		return kodi_utils.ok_dialog(text=line % (ls(32574), str(e)))
