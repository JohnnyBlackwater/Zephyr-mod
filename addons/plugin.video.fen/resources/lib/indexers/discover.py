...
class Discover:
	def __init__(self, params):
		...
		try: self.discover_params = json.loads(kodi_utils.window.getProperty(self.window_id))
	...
	def _clear_property(self):
		kodi_utils.window.clearProperty(self.window_id)
		self.discover_params = {}
		kodi_utils.execute_builtin('Container.Refresh')

	def _set_property(self):
		return kodi_utils.window.setProperty(self.window_id, json.dumps(self.discover_params))
...
def remove_from_history(params):
	...
	dbcon.commit()
	kodi_utils.window.clearProperty(params['data_id'])
	kodi_utils.execute_builtin('Container.Refresh')
