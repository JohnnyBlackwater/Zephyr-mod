...
from modules.utils import jsondate_to_datetime, adjust_premiered_date, make_day, get_datetime, make_thread_list_enumerate, title_key
...
def build_episode_list(params):
	def _process():
		for item in episodes_data:
			try:
				...
				resumetime = get_resumetime(bookmarks, 'episode', tmdb_id, season, episode)
	...
	meta_user_info, watched_indicators, watched_info, show_unaired, is_widget, current_date, adjust_hours, bookmarks = get_episode_info()
...
def build_single_episode(list_type, data):
	def _sort_results(items):
		if list_type_starts_with('next_episode'):
			...
			if nextep_settings['sort_airing_today_to_top']:
				...
				last_weeks_eps = [i for i in items if i[1].getProperty('fen_first_aired') in adjusted_date_list]
				sorted_last_week = sorted(last_weeks_eps, key=lambda i: i[1].getProperty('fen_first_aired'))
				other_eps = [i for i in items if not i in last_weeks_eps]
				sorted_other = sorted(other_eps, key=lambda i: func(i[1].getProperty(sort_key)), reverse=sort_direction)
				items = sorted_last_week + sorted_other
			else: items = sorted(items, key=lambda i: func(i[1].getProperty(sort_key)), reverse=sort_direction)
	...
	def _process(item_position, ep_data):
		try:
			...
			if unaired and list_type_starts_with('next_episode') and not include_unaired: return
			playcount, overlay = get_watched_status(watched_info, string(tmdb_id), season, episode)
			resumetime = get_resumetime(bookmarks, 'episode', tmdb_id, season, episode)
...
def get_episode_info():
	...
	bookmarks = indicators.get_bookmarks('episode')
	return meta_user_info, watched_indicators, watched_info, show_unaired, is_widget, current_date, adjust_hours, bookmarks
