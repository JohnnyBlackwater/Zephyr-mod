...
class Movies:
	...
	def fetch_list(self):
		try:
			...
			kodi_utils.add_items(self.__handle__, self.worker())
			if self.new_page:
					self.new_page.update({'mode': mode, 'action': self.action, 'exit_list_params': self.exit_list_params})
					self.add_dir(self.new_page)
		except: pass
		kodi_utils.set_content(self.__handle__, content_type)
		kodi_utils.end_directory(self.__handle__)
	...
	def build_movie_content(self, item_position, _id):
		try:
			...
			resumetime = get_resumetime(self.bookmarks, 'movie', tmdb_id)
			...
			listitem.setInfo('Video', remove_meta_keys(meta, db_type='movie'))
	...
	def set_meta(self, _id):
		...
		if not meta: return
		playcount, overlay = get_watched_status_movie(self.watched_info, str(meta['tmdb_id']))
	...
	def add_dir(self, url_params, list_name=nextpage_str, iconImage=item_next, isFolder=True):
		...
		kodi_utils.add_item(self.__handle__, url, listitem, isFolder)
