...
class TVShows:
	...
	def build_tvshow_content(self, item_position, _id):
		try:
			...
			total_watched = meta_get('total_watched')
			total_unwatched = meta_get('total_unwatched')
	...
	def set_meta(self, _id):
		...
		if not meta: return
		playcount, overlay, total_watched, total_unwatched = get_watched_status_tvshow(self.watched_info, str(meta['tmdb_id']), meta.get('total_aired_eps'))
