...
class Navigator:
	...
	def imdb_lists(self):
		...
		self.AD({'mode': 'imdb_build_user_lists', 'db_type': 'movies', 'list_name': lst_ins % mov_str}, n_ins % mov_str, 'imdb.png')
		self.AD({'mode': 'imdb_build_user_lists', 'db_type': 'tvshows', 'list_name': lst_ins % tv_str}, n_ins % tv_str, 'imdb.png')
		self._end_directory()

	def search(self):
		search_str, people_str, history_str = ls(32450), ls(32507), ls(32486)
		n_ins, lst_ins = _in_str % (search_str.upper(), '%s'), (_in_str % (search_str, '%s')).replace('[B]', '').replace(': [/B]', ' ')
		movie_history_str, tvshows_history_str, people_history_str = '%s %s' % (mov_str, history_str), '%s %s' % (tv_str, history_str), '%s %s' % (people_str, history_str)
		kw_mov, kw_tv = '%s %s (%s)' % (ls(32064), ls(32092), mov_str), '%s %s (%s)' % (ls(32064), ls(32092), tv_str)
		self.AD({'mode': 'get_search_term', 'db_type': 'movie', 'list_name': lst_ins % mov_str}, n_ins % mov_str, 'search_movie.png', False)
		self.AD({'mode': 'get_search_term', 'db_type': 'tv_show', 'list_name': lst_ins % tv_str}, n_ins % tv_str, 'search_tv.png', False)
		self.AD({'mode': 'people_search.search', 'list_name': lst_ins % people_str}, n_ins % people_str, 'genre_comedy.png', False)
		self.AD({'mode': 'get_search_term', 'search_type': 'imdb_keyword', 'db_type': 'movie', 'list_name': lst_ins % kw_mov}, n_ins % kw_mov, 'search.png', False)
		self.AD({'mode': 'get_search_term', 'search_type': 'imdb_keyword', 'db_type': 'tvshow', 'list_name': lst_ins % kw_tv}, n_ins % kw_tv, 'search.png', False)
		self.AD({'mode': 'search_history', 'action': 'movie', 'list_name': lst_ins % movie_history_str}, n_ins % movie_history_str, 'search.png')
		self.AD({'mode': 'search_history', 'action': 'tvshow', 'list_name': lst_ins % tvshows_history_str}, n_ins % tvshows_history_str, 'search.png')
		self.AD({'mode': 'search_history', 'action': 'people', 'list_name': lst_ins % people_history_str}, n_ins % people_history_str, 'search.png')
		self._end_directory()

	def tools(self):
		from modules.kodi_utils import get_kodi_version
		...
		clean_str, lang_inv_str, art_str = ls(32512), ls(33017), ls(32089)
		...
		self.AD({'mode': 'refresh_artwork', 'list_name': lst_ins % art_str}, n_ins % art_str, 'settings2.png', False)
		...
		if get_kodi_version() >= 18: self.AD({'mode': 'toggle_language_invoker', 'list_name': lst_ins % lang_inv_str}, n_ins % lang_inv_str, 'settings2.png', False)
		self._end_directory()
	...
	def certifications(self):
		...
		lst_ins = self.make_list_name(self.params.get('menu_type'))
	...
	def trakt_mosts(self):
		...
		for most, value in trakt_mosts.items():
			...
			self.AD({'mode': 'navigator.trakt_mosts_duration', 'action': action, 'period_str': most, 'period': value[0], 'menu_type': self.params.get('menu_type'),
	...
	def trakt_mosts_duration(self):
		...
		for duration, urlitem in durations:
			...
			self.AD({'mode': self.params['final_mode'], 'action': self.params['action'], 'period': self.params['period'], 'duration': urlitem,
							'list_name': list_name}, most_str % (period_str.upper(), duration), self.params['iconImage'])
		self._end_directory()
	...
	def adjust_main_lists(self, params=None):
		def db_execute():
			...
			dbcon.commit()
			kodi_utils.window.setProperty('fen_%s_edited' % list_name, json.dumps(li))
		def menu_select(heading, multi_line='true', position_list=False):
			def _builder():
				for item in choice_items:
					...
					try: icon = item.get('iconImage', 'discover.png') if item.get('network_id', None) else os.path.join(icon_directory, item.get('iconImage'))
		...
		def db_execute_shortcut_folder(action='add'):
			...
			dbcon.commit()
			kodi_utils.window.setProperty('fen_%s_shortcut_folder' % menu_name, json.dumps(li))
		def db_execute_add_to_shortcut_folder():
			...
			if action == 'add': dbcon.execute("INSERT INTO navigator VALUES (?, ?, ?)", (menu_name, 'shortcut_folder', json.dumps(li)))
			dbcon.commit()
			kodi_utils.window.setProperty('fen_%s_shortcut_folder' % list_name, json.dumps(li))
		...
		kodi_utils.window.clearProperty('fen_%s_default')
		kodi_utils.window.clearProperty('fen_%s_edited')
		...
		try:
			if method == 'display_edit_menu':
				...
				dbcon.commit()
				kodi_utils.window.setProperty('fen_%s_%s' % (list_name, list_type), json.dumps(li))
				...
				if default_list != new_contents:
					...
					if edited_list:
						...
						kodi_utils.window.setProperty('fen_%s_edited' % list_name, json.dumps(edited_list))
					...
					kodi_utils.window.setProperty('fen_%s_default' % list_name, json.dumps(default_list))
					dbcon.commit()
					dbcon.close()
				else:
				...
				dbcon.commit()
				for item in ('edited', 'default'): kodi_utils.window.clearProperty('fen_%s_%s' % (list_name, item))
		...
		def _process():
			for item_position, item in enumerate(self.default_menu):
				try:
					...
					name = ls(item.get('name', ''))
					icon = item.get('iconImage') if item.get('network_id', '') != '' else os.path.join(icon_directory, item.get('iconImage'))
	...
	def adjust_shortcut_folder_lists(self, params=None):
		...
		try:
			if method == 'display_edit_menu':
				...
				dbcon.commit()
				kodi_utils.window.clearProperty('fen_%s_shortcut_folder' % list_name)
	...
	def _changelog_info(self):
		if kodi_utils.window.getProperty('fen_display_changelog') == 'false': return
		kodi_utils.window.setProperty('fen_display_changelog', 'false')
		addon_version = kodi_utils.addon().getAddonInfo('version')
		changelog_file, changelog_heading = tp('special://home/addons/plugin.video.fen/resources/text/changelog.txt'), 'Fen Changelog  [I](v.%s)[/I]' % addon_version
		kodi_utils.show_text(changelog_heading, file=changelog_file)

	def _db_lists(self, list_name=None):
		list_name = self.list_name if not list_name else list_name
		try:
			default_contents = json.loads(kodi_utils.window.getProperty('fen_%s_default' % list_name))
			try: edited_contents = json.loads(kodi_utils.window.getProperty('fen_%s_edited' % list_name))
		...
		try:
			...
			kodi_utils.window.setProperty('fen_%s_default' % list_name, json.dumps(default_contents))
			kodi_utils.window.setProperty('fen_%s_edited' % list_name, json.dumps(edited_contents))
			return default_contents, edited_contents
		except:
	...
	def _db_lists_shortcut_folder(self, list_name=None):
		list_name = self.list_name if not list_name else list_name
		try:
			contents = json.loads(kodi_utils.window.getProperty('fen_%s_shortcut_folder' % list_name))
			return contents
		except: pass
		try:
			dbcon = database.connect(NAVIGATOR_DB)
			...
			kodi_utils.window.setProperty('fen_%s_shortcut_folder' % list_name, json.dumps(contents))
			return contents
		except:
			return []
