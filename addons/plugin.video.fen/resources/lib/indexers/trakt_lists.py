...
def search_trakt_lists(params):
	...
	if pages > page:
		new_page = int(page) + 1
		kodi_utils.add_dir({'mode': mode, 'search_title': search_title, 'new_page': str(new_page)}, ls(32799), __handle__, iconImage='item_next.png')
...
def build_trakt_list(params):
	...
	original_list, final_listitems = [], []
	original_append = original_list.append
	result = trakt_api.get_trakt_list_contents(user, slug)
	for item in result:
		try:
