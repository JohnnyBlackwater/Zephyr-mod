...
from modules.utils import chunks, make_thread_list
...
class DebridCheck:
	...
	def RD_cache_checker(self):
		...
		single_run = hash_chunk_list.pop(-1)
		self._rd_lookup(single_run) # To refresh token if needed
		if len(hash_chunk_list) > 0:
			threads = list(make_thread_list(self._rd_lookup, hash_chunk_list, Thread))
			[i.join() for i in threads]
