...
def link_folders(service, folder_name, action):
	...
	if action == 'remove':
		...
		dbcon.commit()
		dbcon.close()
		kodi_utils.window.clearProperty(string)
		if service == 'FOLDER':
...
def clean_settings():
	...
	line = '%s[CR]%s[CR]%s'
	kodi_utils.progressDialog.create(ls(32577), line % ('', '', ''))
	kodi_utils.progressDialog.update(0, line % ('  ', '', ''))
	kodi_version = kodi_utils.get_kodi_version()
	...
	for addon in params:
		try:
			...
			kodi_utils.progressDialog.update(percent, line % ('', line2, line3))
		except:
...
def open_MyAccounts(params):
	...
	kodi_utils.sleep(100)
	while kodi_utils.get_visibility('Window.IsVisible(addonsettings)') or kodi_utils.window.getProperty('myaccounts.active') == 'true':
		kodi_utils.sleep(250)
	kodi_utils.sleep(100)
	sync_MyAccounts()
	kodi_utils.sleep(100)
	...
	try:
		...
		if trakt_acct.get('username') not in ('', None):
			if get_setting('trakt_user') != trakt_acct.get('username'):
				...
				kodi_utils.window.setProperty('fen_refresh_trakt_info', 'true')
		else:
...
def clear_all_cache():
	...
	for count, cache_type in enumerate(caches, 1):
		kodi_utils.progressDialog.update(int(float(count) / float(len(caches)) * 100), line % ('', '%s....' % ls(32816), cache_type[1]))
