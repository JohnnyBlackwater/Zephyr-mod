...
def runner(params):
	...
	if action == 'meta.single':
		...
		chosen_list = selectPackItem(pack_choices, params['highlight'], default_icon)
...
def selectPackItem(pack_choices, highlight, icon):
...
class Downloader:
	...
	def downPrep(self):
		if 'meta' in self.params:
			...
			self.meta = json.loads(self.params.get('meta'))
			title = self.getTitle()
			self.db_type = self.meta.get('vid_type')
			self.year = self.meta.get('year')
			self.image = self.meta.get('poster')
			self.image = self.meta.get(art_provider[0]) or self.meta.get(art_provider[1]) or poster_empty
	...
	def getTitle(self):
		...
		if language == 'en': title = self.meta['title']
		else: title = self.meta['original_title']
