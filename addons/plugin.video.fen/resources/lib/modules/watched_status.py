...
TRAKT_DB = kodi_utils.translate_path('special://profile/addon_data/plugin.video.fen/traktcache.db')
ls = kodi_utils.local_string

def get_next_episodes(watched_info, watched_indicators):
	seen = set()
...
def get_resumetime(bookmarks, db_type, tmdb_id, season='', episode=''):
	try: resumetime = str((int(round(float(detect_bookmark(bookmarks, db_type, tmdb_id, season, episode)[0])))/float(100))*2400)
	except: resumetime = '0'
	return resumetime
...
def detect_bookmark(bookmarks, db_type, tmdb_id, season='', episode=''):
	return [(i[1], i[2]) for i in bookmarks if i[0] == str(tmdb_id) and i[3] == season and i[4] == episode][0]

def get_bookmarks(db_type):
	try:
...
def get_in_progress_episodes():
	dbcon = database.connect(WATCHED_DB)
	dbcur = dbcon.cursor()
	dbcur.execute('''SELECT media_id, season, episode, curr_time, resume_point FROM progress WHERE db_type=? ORDER BY rowid DESC''', ('episode',))
	data = dbcur.fetchall()
	episode_list = [{'tmdb_id': i[0], 'season': int(i[1]), 'episode': int(i[2]), 'curr_time': float(i[3]), 'resume_point': i[4]} for i in data]
	return episode_list
...
def mark_as_watched_unwatched_movie(params):
	...
	if watched_indicators == 1:
		...
		else: trakt_watched_unwatched(action, 'movies', tmdb_id)
...
def check_trakt_refresh():
	if kodi_utils.window.getProperty('fen_refresh_trakt_info') == 'true':
		from apis.trakt_api import trakt_sync_activities
		trakt_sync_activities(force_update=True)
		kodi_utils.window.clearProperty('fen_refresh_trakt_info')
