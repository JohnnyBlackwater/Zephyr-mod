...
def results_xml_window_number(window_style=None):
	...
	return {'list': 2000, 'infolist': 2001, 'shift': 2002, 'thumb': 2003}[window_style.split(' ')[0]]
...
def results_sort_order():
	return (
			['quality_rank', 'provider_rank', 'size'], #Quality, Provider, Size
			['quality_rank', 'size', 'provider_rank'], #Quality, Size, Provider
			['provider_rank', 'quality_rank', 'size'], #Provider, Quality, Size
			['provider_rank', 'size', 'quality_rank'], #Provider, Size, Quality
			['size', 'quality_rank', 'provider_rank'], #Size, Quality, Provider
			['size', 'provider_rank', 'quality_rank']  #Size, Provider, Quality
			)[int(get_setting('results.sort_order', '1'))]

def active_scrapers(group_folders=False):
	folders = ['folder1', 'folder2', 'folder3', 'folder4', 'folder5']
	clouds = [('rd', 'provider.rd-cloud'), ('pm', 'provider.pm-cloud'), ('ad', 'provider.ad-cloud')]
	settings = ['provider.external', 'provider.furk', 'provider.easynews']
	...
	if get_setting('provider.folders') == 'true':
		if group_folders: append('folders')
		else: active += folders
	return active
...
def provider_sort_ranks():
	...
	return {'furk': fu_priority, 'easynews': en_priority, 'real-debrid': rd_priority, 'premiumize.me': pm_priority, 'alldebrid': ad_priority,
			'rd-cloud': rd_priority, 'pm-cloud': pm_priority, 'ad-cloud': ad_priority, 'files': 0, 'free': 11}

def sort_to_top(provider):
	return get_setting({'folders': 'results.sort_folders_first', 'rd-cloud': 'results.sort_rdcloud_first', 'pm-cloud': 'results.sort_pmcloud_first',
						'ad-cloud': 'results.sort_adcloud_first'}[provider]) == 'true'
...
def scraping_settings():
	...
	return {'highlight_type': highlight_type, 'hoster_highlight': hoster_highlight, 'torrent_highlight': torrent_highlight,'real-debrid': rd_highlight, 'premiumize': pm_highlight,
			'alldebrid': ad_highlight, 'ad-cloud': debrid_cloud_highlight, 'rd-cloud': debrid_cloud_highlight, 'pm-cloud': debrid_cloud_highlight, 'furk': furk_highlight,
