...
from modules.kodi_utils import translate_path, execute_builtin, sleep, close_all_dialog, hide_busy_dialog, ok_dialog, local_string as ls
...
class Extras(BaseDialog):
	...
	def run(self):
		self.doModal()
		hide_busy_dialog()
		if self.selected: execute_builtin(self.selected)
	...
	def onAction(self, action):
		...
		if action in self.context_actions:
			...
			if focus_id == play_id and self.db_type == 'movie':
				dialogs.playback_choice('movie', self.meta)
	...
	def get_progress(self):
		try: resume_point, curr_time = watched_status.detect_bookmark(watched_status.get_bookmarks('movie'), self.db_type, self.tmdb_id)
	...
	def get_next_episode(self):
		...
		ep_list = watched_status.get_next_episodes(watched_info, self.watched_indicators)
