...
class AllDebridAPI:
	...
	def resolve_magnet(self, magnet_url, info_hash, store_to_cloud, title, season, episode):
		from modules.source_utils import supported_video_extensions, seas_ep_filter, extras_filter, clean_title
		try:
			file_url = None
			correct_files = []
			...
			if season:
				for item in valid_results:
					...
					episode_title = clean_title(title)
	...
	def clear_cache(self):
		try:
			...
			kodi_utils.window.clearProperty('fen_ad_user_cloud')
			dbcon.commit()
			user_cloud_success = True
			# HOSTERS
			dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('fen_ad_valid_hosts',))
			kodi_utils.window.clearProperty('fen_ad_valid_hosts')
			dbcon.commit()
			dbcon.close()
